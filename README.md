...
**rack-audit** — одностраничное веб-приложение для **сравнения вендоров стоек и PDU** по настраиваемому техническому чеклисту. Вы задаете параметры и веса критериев, выставляете оценки каждому вендору (с заметками и фото) и смотрите сводку на дашборде. Данные сохраняются в браузере (`localStorage`); есть выгрузка в Excel.
...
- Небольшие смысловые изменения; не дублировать расчеты скоринга вне `src/scoring.js`, не оставлять неиспользуемые импорты.
