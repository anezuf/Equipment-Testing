  ...
  return <div style={{minHeight:"100vh",background:B.bg,fontFamily:"Inter, system-ui, sans-serif",position:"relative",overflowX:"hidden"}}>
    ...
    {view==="techspecs"&&<div className="view-section-pad" style={{maxWidth:920,margin:"0 auto",padding:"20px 16px"}}>
      ...
      {techSpecs.map((sec,si)=><div key={si} style={{marginBottom:12}}>
        ...
        <div style={{background:"#fff",borderRadius:"0 0 12px 12px",border:`1px solid ${B.border}`,borderTop:"none"}}>
          <div style={{display:"flex",padding:"4px 16px",background:"#F8FAFC",borderBottom:`1px solid ${B.border}`}}>
            <div style={{flex:1,fontSize:10,fontWeight:700,color:B.steel,textTransform:"uppercase",letterSpacing:"0.5px"}}>Параметр</div>
            <div style={{width:1,background:B.border,margin:"0 8px"}}/>
            <div style={{flex:1,fontSize:10,fontWeight:700,color:B.steel,textTransform:"uppercase",letterSpacing:"0.5px"}}>Требование</div>
          </div>
          {sec.items.map((it,ii)=><div key={ii} draggable onDragStart={e=>{e.stopPropagation();e.dataTransfer.setData("text",JSON.stringify({type:"ts-item",si,ii}));}} onDragOver={e=>{e.preventDefault();e.stopPropagation();}} onDrop={e=>{e.preventDefault();e.stopPropagation();try{const d=JSON.parse(e.dataTransfer.getData("text"));if(d.type==="ts-item"&&d.si===si&&d.ii!==ii){const n=[...techSpecs];const s={...n[si],items:[...n[si].items]};const [m]=s.items.splice(d.ii,1);s.items.splice(ii,0,m);n[si]=s;setTechSpecs(n);}}catch{}}} style={{display:"flex",alignItems:"flex-start",gap:0,padding:"8px 16px",borderTop:ii?`1px solid #F1F5F9`:"none",cursor:"grab"}}>
            ...
            <div style={{flex:1,display:"flex",gap:0,minWidth:0,marginLeft:8}}>
              ...
              <div style={{width:1,background:B.border,alignSelf:"stretch",margin:"0 12px",flexShrink:0}}/>
              <textarea value={it.n2||""} onChange={e=>{const v=e.target.value;e.target.style.height="auto";e.target.style.height=e.target.scrollHeight+"px";setTechSpecs(p=>p.map((s,i)=>i===si?{...s,items:s.items.map((x,j)=>j===ii?{...x,n2:v}:x)}:s));}} onFocus={e=>{e.target.style.height="auto";e.target.style.height=e.target.scrollHeight+"px";}} rows={1} placeholder="Введите требование" style={{flex:1,border:"none",background:"none",fontSize:12,color:B.steel,outline:"none",resize:"none",overflow:"hidden",fontFamily:"Inter,system-ui,sans-serif",lineHeight:"1.4",padding:0,minWidth:0}}/>
            </div>
            ...
            {sec.items.map((it,ii)=>{const idx=off+ii;const v=vendors[act]?.scores[idx];const nt=vendors[act]?.notes[idx]||"";const imgs=vendors[act]?.images?.[idx]||null;const hasImgs=imgs&&imgs.length>0;const isExp=noteOpen===idx||noteOpen===-999;const isReq=it.w>=1;const hasNote=nt&&nt.trim()!==""&&nt.trim()!=="<br>"&&nt.trim()!=="<div><br></div>";
              return <div key={ii} style={{borderTop:ii?`1px solid #F1F5F9`:"none"}}>
                <div style={{display:"flex",alignItems:"center",padding:"8px 16px",gap:10,flexWrap:"wrap"}}>
                  <div className="input-item-name" style={{flex:"1 1 150px",display:"flex",alignItems:"flex-start",gap:6,minWidth:0}}><span style={{fontSize:12,color:B.graphite,whiteSpace:"normal",wordBreak:"break-word",lineHeight:"1.4",textAlign:"left",minWidth:0}}>{it.n}</span><span style={wbBadge(it.w)}>{it.w===2?"★!":it.w===1?"★":"☆"}</span></div>
          ...
          {sortedIdx.map(i=>{const v=vendors[i];
            ...
            {sections.map((s,si)=>{
              ...
              return <div key={si} style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}>
                <div className="sec-bar-label" style={{width:120,minWidth:0,fontSize:9,color:"#334155",fontWeight:500,textAlign:"right",flexShrink:0,whiteSpace:"normal",wordBreak:"break-word"}} title={s.n}>{s.n}</div>
